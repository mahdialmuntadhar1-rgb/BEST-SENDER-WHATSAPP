...
contacts.get('/', requireAuth, async (c: AuthContext) => {
...
contacts.post('/', requireAuth, async (c: AuthContext) => {
  const db = new D1Client(c.env.DB);
...
contacts.put('/:id', requireAuth, async (c: AuthContext) => {
  const db = new D1Client(c.env.DB);
...
contacts.delete('/:id', requireAuth, async (c: AuthContext) => {
  const db = new D1Client(c.env.DB);
  const id = c.req.param('id');
...
contacts.delete('/bulk', requireAuth, async (c: AuthContext) => {
  const db = new D1Client(c.env.DB);
...
contacts.get('/governorates/counts', requireAuth, async (c: AuthContext) => {
  const db = new D1Client(c.env.DB);
